...
export const appRouter = router({
  ...
  ocr: router({
    ...
    batchScan: publicProcedure
      .input(z.object({
        images: z.array(z.string().min(100).max(7_000_000)).min(1).max(4),
      }))
      .mutation(async ({ input }) => {
    ...
    scanPair: publicProcedure
      .input(z.object({
        registrationBase64: z.string().min(100).max(7_000_000),
        greenCardBase64:    z.string().min(100).max(7_000_000),
      }))
      .mutation(async ({ input }) => {
