...
function drawCar(
  ...
  wheelPositions.forEach(([wx, wy]) => {
    ctx.beginPath();
    ctx.ellipse(wx , wy , h * 0.18, h * 0.14, 0, 0, Math.PI * 2);
    ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#666';
    ctx.beginPath();
    ctx.ellipse(wx , wy , h * 0.08, h * 0.06, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = '#222';
  });
...
function drawScooter(ctx, cx, cy, W, H, color, label, angle) {
  drawMotorcycle(ctx, cx, cy, W * 0.75, H * 0.75, color, label, angle);
}

function drawEscooter(ctx, cx, cy, W, _H, color, label, angle) {
  ctx.save();
  ctx.translate(cx, cy);
  ctx.rotate(angle);
  const w = W * 0.4, h = w;
...
function drawVan(ctx, cx, cy, W, H, color, label, angle) {
  ctx.save();
  ctx.translate(cx, cy);
  ctx.rotate(angle);
...
function directionToAngle(dir, trafficSide = 'right') {
...
function drawRoadScene(
  ...
  const drawStraightRoad = (horizontal, x1, y1, x2, y2) => {
    ctx.fillStyle = ROAD;
    if (horizontal) {
