// boom.contact — Service Worker v20
// Full PWA offline mode: CacheFirst for shell, NetworkFirst for API, CacheFirst for tiles
// Backward-compatible with v19 — all old caches purged on activate

const CACHE_NAME = 'boom-contact-v20';
    ...
    caches.keys().then((keys) =>
      Promise.all(
        keys.filter((k) => !KEEP.includes(k)).map((k) => {
          console.log('[SW v20] Suppression cache obsolete:', k);
          return caches.delete(k);
        })
      )
    ).then(() => {
      console.log('[SW v20] Actif — anciens caches supprimes');
...
self.addEventListener('message', (event) => {
  ...
  if (event.data === 'GET_VERSION') {
    event.source?.postMessage({ version: 'v20', cache: CACHE_NAME });
  }
