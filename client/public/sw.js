// boom.contact — Service Worker v27
...
const CACHE_NAME = 'boom-contact-v27';
    ...
    caches.keys().then((keys) =>
      Promise.all(
        keys.filter((k) => !KEEP.includes(k)).map((k) => {
          console.log('[SW v27] Suppression cache obsolete:', k);
          return caches.delete(k);
        })
      )
    ).then(() => {
      console.log('[SW v27] Actif — anciens caches supprimes');
...
self.addEventListener('message', (event) => {
  ...
  if (event.data === 'GET_VERSION') {
    event.source?.postMessage({ version: 'v27', cache: CACHE_NAME });
  }
