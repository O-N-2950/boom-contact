// boom.contact — Service Worker v19
// Full PWA offline mode: CacheFirst for shell, NetworkFirst for API, CacheFirst for tiles
// Backward-compatible with v18 — all old caches purged on activate

const CACHE_NAME = 'boom-contact-v19';
    ...
    caches.keys().then((keys) =>
      Promise.all(
        keys.filter((k) => !KEEP.includes(k)).map((k) => {
          console.log('[SW v19] Suppression cache obsolete:', k);
          return caches.delete(k);
        })
      )
    ).then(() => {
      console.log('[SW v19] Actif — anciens caches supprimes');
...
self.addEventListener('message', (event) => {
  ...
  if (event.data === 'GET_VERSION') {
    event.source?.postMessage({ version: 'v19', cache: CACHE_NAME });
  }
