// boom.contact — Service Worker v14
// Full PWA offline mode: CacheFirst for shell, NetworkFirst for API, CacheFirst for tiles
// Backward-compatible with v13 — all old caches purged on activate

const CACHE_NAME = 'boom-contact-v14';
    ...
    caches.keys().then((keys) =>
      Promise.all(
        keys.filter((k) => !KEEP.includes(k)).map((k) => {
          console.log('[SW v14] Suppression cache obsolete:', k);
          return caches.delete(k);
        })
      )
    ).then(() => {
      console.log('[SW v14] Actif — anciens caches supprimes');
...
self.addEventListener('message', (event) => {
  ...
  if (event.data === 'GET_VERSION') {
    event.source?.postMessage({ version: 'v14', cache: CACHE_NAME });
  }
