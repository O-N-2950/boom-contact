// boom.contact — Service Worker v34
...
const CACHE_NAME = 'boom-contact-v34';
    ...
    caches.keys().then((keys) =>
      Promise.all(
        keys.filter((k) => !KEEP.includes(k)).map((k) => {
          console.log('[SW v34] Suppression cache obsolete:', k);
          return caches.delete(k);
        })
      )
    ).then(() => {
      console.log('[SW v34] Actif — anciens caches supprimes');
...
self.addEventListener('message', (event) => {
  ...
  if (event.data === 'GET_VERSION') {
    event.source?.postMessage({ version: 'v34', cache: CACHE_NAME });
  }
