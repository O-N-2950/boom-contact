...
export function CGUModal({ onAccept, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-end justify-center" style={{ background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(8px)' }} onClick={e => e.target === e.currentTarget && onClose()}>
      <div ref={modalRef} role="dialog" aria-modal="true" aria-label="Conditions générales" className="w-full max-w-[480px] flex flex-col bg-[#0E0E18]" style={{ border: '1px solid rgba(255,255,255,0.25)', borderRadius: '24px 24px 0 0', maxHeight: '90svh', animation: 'slideUp 0.3s ease' }}>
        {/* Handle */}
        <div className="flex justify-center" style={{ padding: '12px 0 0' }}>
          <div className="w-10 h-1 rounded-sm" style={{ background: 'rgba(255,255,255,0.15)' }} />
        </div>

        {/* Header */}
        <div style={{ padding: '16px 24px 0' }}>
          <div className="flex items-center gap-2.5 mb-1">
            <div className="flex items-center justify-center text-sm w-7 h-7 rounded-[7px]"  style={{ background: 'var(--boom)' }}>💥</div>
          ...
          {/* Tabs */}
          <div className="flex mb-0"  style={{ borderBottom: '1px solid rgba(255,255,255,0.25)' }}>
            {([['cgu', t('cgu.tab_cgu')], ['privacy', t('cgu.tab_privacy')]] as const).map(([id, label]) => (
              <button key={id} onClick={() => setTab(id)} className="bg-none border-0 cursor-pointer text-xs px-4 py-2" style={{ color: tab === id ? 'var(--boom)' : 'rgba(240,237,232,0.45)', borderBottom: tab === id ? '2px solid var(--boom)' : '2px solid transparent', fontWeight: tab === id ? 700 : 400 }}>{label}</button>
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {tab === 'cgu' ? (
            <div className="text-xs leading-[1.75]" style={{ color: 'rgba(240,237,232,0.75)' }}>
              <div className="font-bold text-[13px] mb-2" style={{ color: 'var(--text)' }}>{t('cgu.cgu_title')}</div>
              <div className="text-[10px] mb-4 opacity-70"  style={{ fontFamily: 'monospace' }}>{t('cgu.cgu_version')}</div>
              {cguSections.map((s, i) => (
                <div key={i} className="mb-4">
                  <div className="font-bold mb-1 text-xs" style={{ color: 'var(--text)' }}>{s.title}</div>
                  <div>{s.text}</div>
                </div>
              ))}
            </div>
            ...
            <div className="text-xs leading-[1.75]" style={{ color: 'rgba(240,237,232,0.75)' }}>
              <div className="font-bold text-[13px] mb-2" style={{ color: 'var(--text)' }}>{t('cgu.privacy_title')}</div>
        ...
        {/* Bottom action area */}
        <div className="shrink-0 px-6 pt-4 pb-8" style={{ borderTop: '1px solid rgba(255,255,255,0.25)' }}>
          {/* Email */}
          <div className="mb-3.5">
            ...
            <input
              ...
              className="input-boom w-full" />
          </div>

          {/* CGU consent (required) */}
          <label className="flex items-start gap-3 mb-2.5 cursor-pointer">
            <input type="checkbox" checked={consentCGU} onChange={() => setConsentCGU(!consentCGU)} aria-label="Accepter les conditions générales" required className="w-5 h-5 rounded-[5px] shrink-0 mt-px cursor-pointer" style={{ accentColor: 'var(--boom)' }} />
            <div className="text-xs leading-normal opacity-80" >
              ...
              {t('cgu.consent_cgu').split(/(<cgu>.*?<\/cgu>|<privacy>.*?<\/privacy>)/g).map((part, i) => {
                if (part.startsWith('<cgu>')) return (
                  <span key={i} onClick={() => setTab('cgu')} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setTab('cgu'); } }} role="button" tabIndex={0} className="cursor-pointer underline" style={{ color: 'var(--boom)' }}>
                    {part.replace(/<\/?cgu>/g, '')}
                  </span>
                );
                if (part.startsWith('<privacy>')) return (
                  <span key={i} onClick={() => setTab('privacy')} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setTab('privacy'); } }} role="button" tabIndex={0} className="cursor-pointer underline" style={{ color: 'var(--boom)' }}>
          ...
          <button onClick={handleSubmit} disabled={!canProceed || loading} className="w-full p-4 rounded-xl border-0 font-bold transition-all duration-200 text-[15px]" style={{ background: canProceed ? 'var(--boom)' : 'rgba(255,255,255,0.08)', color: canProceed ? '#fff' : 'rgba(255,255,255,0.6)', cursor: canProceed ? 'pointer' : 'not-allowed' }}>
